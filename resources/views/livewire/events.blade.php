        ...
        @if(count($upcomingEvents) > 0)
            <div class="event-list">
                @foreach($upcomingEvents as $event)
                    <x-events.event :event="$event" :expanded="$this->isExpanded($event->id)" :is-past-event="false" wire:click="toggleEvent({{ $event['id'] }})" />
                @endforeach
            </div>
        @else
            <div class="no-events">
        ...
        <section class="section" id="past-events">
            ...
            <div class="event-list past-events">
                @foreach($pastEvents as $event)
                    <x-events.event :event="$event" :expanded="$this->isExpanded($event->id)" :is-past-event="true" wire:click="toggleEvent({{ $event['id'] }})" />
